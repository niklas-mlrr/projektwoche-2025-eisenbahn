...
class TrainHubGUI:
    ...
    def create_basic_motor_tab(self, parent):
        ...
        self.arduino_ref_frame = tk.Frame(ar_frame, bg='#263238', highlightthickness=1, highlightbackground='#455A64')
        self.arduino_ref_frame.pack(padx=0, pady=(4, 0), anchor='center')

        _hdr = tk.Frame(self.arduino_ref_frame, bg='#37474F')
        _hdr.pack(padx=6, pady=2)
        tk.Label(_hdr, text="Range", bg='#37474F', fg='#CFD8DC', font=('Arial', 8, 'bold')).pack(side=tk.LEFT, padx=(6, 8), pady=2)
        tk.Label(_hdr, text="Rate",  bg='#37474F', fg='#CFD8DC', font=('Arial', 8, 'bold')).pack(side=tk.LEFT)

        _grid = tk.Frame(self.arduino_ref_frame, bg='#263238')
        _grid.pack(pady=2)

        _row1 = tk.Frame(_grid, bg='#263238')
        _row1.pack()
        tk.Label(_row1, text=">= 759", bg='#263238', fg='#ECEFF1', font=('Consolas', 9)).pack(side=tk.LEFT, padx=(6, 8), pady=1)
        tk.Label(_row1, text="+5.0",  bg='#263238', fg='#4CAF50', font=('Consolas', 9, 'bold')).pack(side=tk.LEFT, pady=1)

        _row2 = tk.Frame(_grid, bg='#2E3B43')
        _row2.pack()
        tk.Label(_row2, text="711..760", bg='#2E3B43', fg='#ECEFF1', font=('Consolas', 9)).pack(side=tk.LEFT, padx=(6, 8), pady=1)
        tk.Label(_row2, text="+2.0",     bg='#2E3B43', fg='#4CAF50', font=('Consolas', 9, 'bold')).pack(side=tk.LEFT, pady=1)

        _row3 = tk.Frame(_grid, bg='#263238')
        _row3.pack()
        tk.Label(_row3, text="660..710", bg='#263238', fg='#ECEFF1', font=('Consolas', 9)).pack(side=tk.LEFT, padx=(6, 8), pady=1)
        tk.Label(_row3, text="0.0",      bg='#263238', fg='#B0BEC5', font=('Consolas', 9, 'bold')).pack(side=tk.LEFT, pady=1)

        _row4 = tk.Frame(_grid, bg='#2E3B43')
        _row4.pack()
        tk.Label(_row4, text="610..659", bg='#2E3B43', fg='#ECEFF1', font=('Consolas', 9)).pack(side=tk.LEFT, padx=(6, 8), pady=1)
        tk.Label(_row4, text="-2.0",     bg='#2E3B43', fg='#F44336', font=('Consolas', 9, 'bold')).pack(side=tk.LEFT, pady=1)

        _row5 = tk.Frame(_grid, bg='#263238')
        _row5.pack()
        tk.Label(_row5, text="0..609 and negatives", bg='#263238', fg='#ECEFF1', font=('Consolas', 9)).pack(side=tk.LEFT, padx=(6, 8), pady=1)
        tk.Label(_row5, text="-5.0",                   bg='#263238', fg='#F44336', font=('Consolas', 9, 'bold')).pack(side=tk.LEFT, pady=1)
